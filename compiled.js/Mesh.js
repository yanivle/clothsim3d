...
export default class Mesh {
    ...
    static BuildPyramid() {
        ...
        let p3 = new Vec3(0.5, 0, Math.sqrt(3));
        let base = new Triangle(p1, p2, p3, mesh.color);
        let base_center = base.center;
        base.translate(base_center.negate());
        mesh.triangles.push(base);
        let p4 = new Vec3(0, base_center.len, 0);
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < i; j++) {
                let pa = [p1, p2, p3][i];
                let pb = [p1, p2, p3][j];
                let t = new Triangle(pa, pb, p4, mesh.color);
    ...
    static BuildSphere(iterations = 3) {
